...
// 0: = Side (e.g. BLUFOR), or group name(s) as string array (e.g.
// ["mrGroup1","myGroup2"])
...
// FAULT CHECK
// 10 seconds into the mission we check if any groups were found. If not, exit
...
// Every 6 seconds the server will check to see if the number of casualties
// sustained within the group(s) has reached the percentage specificed in the
// variable _pc. If the cap has been reached, the loop will exit to trigger
// the ending.
