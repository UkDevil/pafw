...
// The default class picked when no corresponding entry is found in the
// _unitClasses array
_defaultclass = "r";
// The unit classes and their corresponding Assign Gear Component type
_unitClasses = [
    ["_officer_"    ,   "co"    ],
    ["_colonel_"    ,   "co"    ],
    ["_sl_"         ,   "dc"    ],
...
// LOOP THROUGH AI UNITS AND ASSIGN GEAR
{
    ...
    // Check if the unit was already touched by the Assign Gear Component
    if (!(_unit getvariable ["f_var_assignGear_done", false]) && {!(_unit in playableUnits) && (_unit isKindOf "Man")}) then {
        ...
        if (_faction in _unitFactions) then {
            _known = false;
            {
                ...
                // If the unit's classname corresponds to a class in the
                // assignment array, set its gear accordingly
                if (_known) exitWith {
            ...
            // If the faction is not allowed, set the assignGear variable to
            // true to ignore the unit from now on
